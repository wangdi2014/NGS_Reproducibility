...
set.seed(13342143)
sessionInfo()
setwd("/Users/wjidea/Google\ Drive/Graduate_Study/9_Stats_Bioinfo/9_Bioinfo_Study/NGS_2015/NGS_Reproducibility/")
...
T <- FALSE
T
Bmori.dat <- read.csv("Bmori-data.csv", header = T)
head(Bmori.dat) 
...
boxplot(log2(Bmori.dat$rpkm.m70[Bmori.dat$rpkm.m70 >= 1]) ~ Bmori.dat$chr[Bmori.dat$rpkm.m70 >= 1],  pch = 21, notch = FALSE, col = c("red", rep("grey", 27)) ), na.rm = FALSE, las = 1, ylim = c(-1, 15), main = expression(paste(italic("Expr"), " knockdown (male)")), ylab = expression(paste("log"[2]," expression (RPKM)")), outline = FALSE)
par(mfrow = c(1, 1))
